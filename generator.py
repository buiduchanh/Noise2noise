...
class NoisyImageGenerator(Sequence):
    ...
    def __getitem__(self, idx):
        ...
        while True:
            ...
            noise_path = str(image_path).replace('clean','noise')
            ...
            if h >= image_size and w >= image_size:
                ...
                clean_patch = image_pre.load_img(str(image_path), color_mode='rgb', target_size=(image_size, image_size))
                clean_patch = image_pre.img_to_array(clean_patch).astype('float32')[:,:,::-1]
                
                x[sample_id] = self.source_noise_model(str(noise_path))
...
class ValGenerator(Sequence):
    def __init__(self, image_dir, val_noise_model, image_size = None):
        image_suffixes = (".jpeg", ".jpg", ".png", ".bmp")
        ...
        for image_path in image_paths:
            ...
            clean_patch = image_pre.load_img(str(image_path), color_mode='rgb', target_size=(image_size, image_size))
            y = image_pre.img_to_array(clean_patch).astype('float32')[:,:,::-1]

            x = val_noise_model(noise_path)
